...
contract TLToken is ERC20{
    ...
    uint256 public feePercent = 3; // 3% 手续费
    uint256 public profitTriggerPercent = 10; // 10% 盈利触发
    uint256 public buyBackPercent = 5; // 5% 回购销毁
    uint256 public rewardLpPercent = 5; // 5% 分给LP
    uint256 public lpWithdrawFeePercent = 3; // 撤出LP 3% 手续费
    ...
    // 新增： 记录用户的买入 USDT 和代币数量
    mapping (address => uint256) public _userBuyUsdtAmount;
    mapping (address => uint256) public _userBuyTokenAmount;

    constructor(address _usdtAddress, address _routerAddress) ERC20("TL", "TL") {
        uint256 total = 21000000 * 10**18; // 2100万
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        if (isWhiteListed[from]) {
            require(amount <= (isTeamLeader(from) ? 60 * 10**18 : 30 * 10**18), "Exceed white list limit");
        }
        ...
        // 如果是卖出操作，计算盈利并触发回购销毁和奖励 LP
        if (to == uniswapV2Pair) {
            uint256 profit = calculateProfit(from, amount);
            if (profit >= profitTriggerPercent) {
                uint256 buyBackAmount = profit.mul(buyBackPercent).div(100);
                uint256 rewardLpAmount = profit.mul(rewardLpPercent).div(100);
                _buyBackAndBurn(buyBackAmount);
                _rewardLpHolders(rewardLpAmount);
            }
        }
    ...
    // 计算用户的盈利
    function calculateProfit(address user, uint256 sellAmount) internal view returns (uint256) {
        uint256 buyUsdtAmount = _userBuyUsdtAmount[user];
        uint256 sellUsdtAmount = getSellUsdtAmount(sellAmount);
        if (sellUsdtAmount > buyUsdtAmount) {
            return sellUsdtAmount.sub(buyUsdtAmount).mul(100).div(buyUsdtAmount); // 盈利百分比
        }
        return 0;
    }
    ...
    function getSellUsdtAmount(uint256 sellAmount) internal view returns (uint256) {
        (uint256 reserveU, uint256 reserveT, ) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        return uniswapV2Router.getAmountOut(sellAmount, reserveT, reserveU);
    }

    // 回购并销毁代币
    function _buyBackAndBurn(uint256 amount) internal {
        _balances[_destroyAddress] = _balances[_destroyAddress].add(amount);
        emit Transfer(address(this), _destroyAddress, amount);
    }

    // 奖励 LP 持有者
    function _rewardLpHolders(uint256 amount) internal {
        // 分发给 LP 持有者的逻辑可以根据具体需求实现
        // 这里可以通过遍历 LP 持有者并按比例分配奖励
    }

    // 自动将代币和 USDT 添加到流动性池中
    ...
    function isTeamLeader(address user) public view returns (bool) {
        return user == _owner; // 团队长是合约所有者
    }

    function withdrawLP(uint256 lpAmount) public {
        uint256 fee = lpAmount.mul(lpWithdrawFeePercent).div(100);
        uint256 amountAfterFee = lpAmount.sub(fee);

        _balances[_destroyAddress] = _balances[_destroyAddress].add(fee);
        emit Transfer(msg.sender, _destroyAddress, fee);

        _balances[msg.sender] = _balances[msg.sender].sub(lpAmount);
